...
> [!IMPORTANT]
>
> **在Python中，双斜杠 `//` 是整数除法运算符，用于执行整数除法并返回结果的整数部分，即执行地板除法（向下取整）。**
>
> 这也是去除数据类型后的必要操作，C语言等有着显式数据类型的编程语言，`/`就代表Python中`/`和`//`的作用，根据不同的数据类型做判断就可以。而Python模糊了整数和小数的边界，使得除法运算不管整数还是小数，运算结果只要有小数，就是小数。
